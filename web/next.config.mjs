...
const nextConfig = {
  ...
  serverExternalPackages: [
    ...
    "kysely",
    // tiktoken ships `tiktoken_bg.wasm` as a sibling file that
    // tiktoken.cjs locates at runtime via __dirname + fs.readFileSync.
    // Bundlers don't carry the .wasm alongside the .cjs that way, so
    // we need Node's native require to load tiktoken from node_modules
    // unbundled. Next.js 15.x Turbopack did not honour this flag for
    // workspace-transitive deps; 16.1 fixed that — keeping
    // serverExternalPackages here is what makes tiktoken WASM work in
    // the web direct-write tokenisation path.
    "tiktoken",
    "@anthropic-ai/tokenizer",
    // PGlite ships WASM (postgres.wasm/.data) that must be loaded unbundled
    // from node_modules at runtime, like tiktoken above.
    "@electric-sql/pglite",
  ],
  poweredByHeader: false,
  basePath: env.NEXT_PUBLIC_BASE_PATH,
  turbopack: {
    // Keep Turbopack rooted at the repo so unrelated lockfiles under /root
    // do not break workspace alias resolution during standalone builds.
