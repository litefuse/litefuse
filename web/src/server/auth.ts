...
const staticProviders: Provider[] = [
  CredentialsProvider({
    ...
    async authorize(credentials, _req) {
      ...
      // Multi-tenant SSO enforcement is an Enterprise Edition feature; the OSS
      // build always falls through to credential-based authentication.
...
export async function getAuthOptions(): Promise<NextAuthOptions> {
  // Multi-tenant dynamic SSO providers are an Enterprise Edition feature; OSS
  // only exposes the statically configured providers.
  const providers: Provider[] = [...staticProviders];
    ...
    callbacks: {
      ...
      async signIn({ user, account, profile }) {
        return instrumentAsync({ name: "next-auth-sign-in" }, async (span) => {
          ...
          // Multi-tenant SSO enforcement / per-domain provider validation are
          // Enterprise Edition features and are skipped in this build.
