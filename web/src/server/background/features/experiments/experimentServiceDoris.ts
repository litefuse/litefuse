import { type DatasetItemDomain, type Prisma } from "@langfuse/shared";
import {
  type ChatMessage,
  ...
  QueueName,
  eventTypes,
  type ExperimentCreateEventSchema,
  fetchLLMCompletion,
...
`;
  ...
  const rows = await queryDoris<{ id: string }>({
    query,
    params: {
      projectId,
      runId,
...
async function processItem(
  ...
  // Use unified trace ID to avoid creating duplicate traces between PostgreSQL and Doris
  ...
  const queue = getPgBossQueue(QueueName.DatasetRunItemUpsert);
  await queue.send(QueueJobs.DatasetRunItemUpsert, {
    projectId,
    datasetItemId: datasetItem.id,
...
export const createExperimentJobDoris = async ({
  event,
}: {
  event: z.infer<typeof ExperimentCreateEventSchema>;
}) => {
  const startTime = Date.now();
  logger.info("Processing experiment create job with Doris batching", event);
...
// In error cases (config errors), we always create traces in the Doris execution path since PostgreSQL execution
