...
export async function scheduleExperimentObservationEvals(
  ...
  try {
    ...
    // 3. Schedule evals.
    // The generation just landed in events_full via internal tracing,
    // so its start_time_date partition is today's UTC date. Eval
    // execution re-reads from events_full using this key.
    const startTimeDate = new Date().toISOString().split("T")[0];
