...
import type { QueueJobData } from "../../queues/types";
...
export const handleBatchActionJob = async (
  batchActionJob: QueueJobData<QueueName.BatchActionQueue>,
) => {
  ...
  if (span) {
    ...
    span.setAttribute(
      "messaging.pgboss.job.input.actionId",
  ...
  if (
    ...
    const evalCreatorQueue = getPgBossQueue(QueueName.CreateEvalQueue);
    ...
    for await (const record of dbReadStream) {
      if (
        ...
        await evalCreatorQueue.send(QueueJobs.CreateEvalJob, payload);
        count++;
      } else if (
