...
import type { QueueJobLike } from "../../queues/types";
import {
...
export const handleMixpanelIntegrationProjectJob = async (
  ...
  if (span) {
    span.setAttribute("messaging.pgboss.job.input.jobId", job.data.id);
    span.setAttribute("messaging.pgboss.job.input.projectId", projectId);
  }
  ...
  const executionConfig: MixpanelExecutionConfig = {
    ...
    // Start from 2000-01-01 if no lastSyncAt; 1970-01-01 causes subtle DateTime bugs in some backends.
