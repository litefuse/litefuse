...
export const getObservationStream = async (props: {
  ...
  const query = `
      ...
      SELECT
        ...
        t.name as traceName,
        t.tags as traceTags,
        t.start_time as traceTimestamp,
        t.user_id as userId,
        s.scores_avg,
        s.score_categories
      FROM events_full o
        LEFT JOIN events_full t ON t.trace_id = o.trace_id AND t.project_id = o.project_id AND t.parent_span_id = ''
        LEFT JOIN scores_agg s ON s.trace_id = o.trace_id AND s.observation_id = o.span_id
      WHERE o.parent_span_id != ''
        AND ${appliedObservationsFilter.query}
        ${search.query}
  ...
  type ObservationRow = Omit<ObservationRecordReadType, "metadata"> & {
    metadata_names: unknown;
    metadata_values: unknown;
    scores_avg: string | undefined;
    score_categories: string | undefined;
  ...
  const processObservationRow = async (
    ...
    const { metadata_names, metadata_values, ...rest } = bufferedRow;
    ...
    return getChunkWithFlattenedScores(
      [
        {
          ...convertObservation(observationRecord, {
            truncated: false,
