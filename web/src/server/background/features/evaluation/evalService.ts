...
import {
  ...
  type DatasetRunItemUpsertEventType,
...
export const createEvalJobs = async ({
  ...
  if (span) {
    span.setAttribute("messaging.pgboss.job.input.projectId", event.projectId);
  }
  ...
  // This should increase throughput, but will also put more pressure on Doris.
  ...
  for (const config of configs) {
    ...
    if (traceExists && (!isDatasetConfig || Boolean(datasetItem))) {
      ...
      const evalQueue = getPgBossQueue(QueueName.EvaluationExecution);
      await evalQueue.send(
        ...
        {
          startAfter: config.delay ? config.delay / 1000 : undefined,
...
export async function executeLLMAsJudgeEvaluation({
  ...
  return instrumentAsync(
    ...
    async (span) => {
      ...
      // Direct-write the score event through processEventBatch (web
      // direct-write path; no S3 staging or BullMQ enqueue anymore).
      try {
        ...
        traceException(e);
        throw new Error(`Failed to direct-write score ${scoreId}`);
      }
...
export async function extractVariablesFromTracingData({
  ...
  // We do not cache dataset items because Postgres is cheaper than Doris.
