...
async function processEvents(
  ...
  // ProjectDelete is on pg-boss now; enqueue per-event. Batch sizing
  // is preserved only to keep the progress log readable.
  for (let i = 0; i < events.length; i += batchSize) {
    const batch = events.slice(i, i + batchSize);

    for (const event of batch) {
      await enqueuePgBossJob(queueName, jobName, {
        payload: event,
        timestamp: new Date(),
      });
    }
