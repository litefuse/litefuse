 ...
 *
 * Usage:
 *   LITEFUSE_HOST=https://cloud.litefuse.ai \
...
const LITEFUSE_HOST = process.env.LITEFUSE_HOST;
const ADMIN_API_KEY = process.env.ADMIN_API_KEY;

if (!LITEFUSE_HOST) {
  console.error("Error: LITEFUSE_HOST environment variable is required");
  process.exit(1);
}
if (!ADMIN_API_KEY) {
...
const API_URL = `${LITEFUSE_HOST.replace(/\/+$/, "")}/api/admin/ingestion-replay`;
