...
export const webhookProcessor = async (
  job: QueueJobLike<QueueName.WebhookQueue>,
) => {
  if (job.name !== QueueJobs.WebhookJob) return;
  ...
  await instrumentAsync(
    ...
    async () => {
      try {
        await executeWebhook(job.data.payload);
      } catch (error) {
