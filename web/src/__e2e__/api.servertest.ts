...
// Litefuse Lightweight: /api/public/ingestion is removed; covered by OTel path.
describe.skip("Ingestion Pipeline", () => {
  ...
  it("ingest a trace", async () => {
    ...
    await waitForExpect(
      async () => {
        // we need a second call to the public API with the API key, so that it is stored in app cache
        // first call (ingestion above) generates the new, fast API hash
        // second call (below) stores the API key in app cache
        ...
        const cachedKeys = getLocalAppCacheKeysSnapshot().filter((key) =>
          key.startsWith("api-key:"),
        );
        expect(cachedKeys).toHaveLength(1);
        const llmApiKey = OrgEnrichedApiKey.parse(
          await getAppCacheValueForTest(cachedKeys[0]),
        );
...
describe("Prompts endpoint", () => {
  it("creates and returns a prompt", async () => {
    ...
    const promptCacheEpoch = await getAppCacheValueForTest<string>(
      `prompt_cache_epoch:${projectId}`,
    );
    expect(promptCacheEpoch).toBeTruthy();

    const cacheEntry = await getAppCacheEntry(
      `prompt:${projectId}:${promptCacheEpoch}:${promptName}:${validatedPrompt.labels[0]}`,
    );

    expect(cacheEntry).not.toBeNull();
    expect(cacheEntry?.value).toEqual({
      ...validatedPrompt,
      isActive: null, // deprecated isActive is being patched at the endpoint handler level; in app cache it is still null as in DB
    });
  });
});
