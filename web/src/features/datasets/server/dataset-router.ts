...
/**
 * Determines whether the given filters require Dataset Run Items (DRI) metrics from Doris.
 *
    ...
    .query(async ({ input }) => {
      ...
      const allRuns = runsWithMetrics.map((run) => {
        return {
          id: run.id,
          name: run.name,
          // Use Doris metrics if available, otherwise use defaults for runs without dataset_run_items_rmt
