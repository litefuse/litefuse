...
export async function telemetry() {
  try {
    ...
    if (env.NEXT_PUBLIC_LITEFUSE_CLOUD_REGION !== undefined) return;
  ...
  try {
    ...
    posthog.capture({
      ...
      properties: {
        ...
        eeLicenseKey: env.LITEFUSE_EE_LICENSE_KEY,
        langfuseCloudRegion: env.NEXT_PUBLIC_LITEFUSE_CLOUD_REGION,
        $set: {
