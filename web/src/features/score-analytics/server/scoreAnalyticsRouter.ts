...
import { buildSamplingExpression } from "./queryHelpers";
...
export const scoreAnalyticsRouter = createTRPCRouter({
  ...
  getScoreIdentifiers: protectedProjectProcedure
    ...
    .query(async ({ input }) => {
      ...
      // Query for distinct score names, data types, and sources
  ...
  estimateScoreComparisonSize: protectedProjectProcedure
    ...
    .query(async ({ input }) => {
      ...
      // Determine if sampling will be used
      const willSample =
  ...
  getScoreComparisonAnalytics: protectedProjectProcedure
    ...
    .query(async ({ input }) => {
      ...
      // Reuse the shared sampling expression builder (Doris-compatible)
