...
export async function executeQuery(
  ...
  // as Date objects. Convert to the ISO/number shape the frontend expects.
  const converted = rows.map((row) => {
    const out: Record<string, unknown> = {};
    for (const [key, value] of Object.entries(row)) {
      if (value instanceof Date) {
        // Output ISO 8601 format with timezone indicator so the frontend
  ...
  // Doris doesn't support WITH FILL. Fill time gaps with zeros so continuous
  // time series charts have an unbroken X axis. Skip gap-filling when ORDER
  // BY is specified (upstream WITH FILL is also skipped in that case).
...
/**
 * Fill time series gaps with zero values at the application layer (Doris
 * has no WITH FILL equivalent).
 *
...
function fillTimeSeriesGaps(
  ...
  // and zero metrics for time buckets without any data. Fill rows get
  // default values ("" for strings, 0 for numbers). The frontend's
  // DashboardWidget converts empty-string dimensions to "n/a", creating
  // a flat zero-value series that extends the X-axis to the full from/to
  // range.
  const result: Record<string, unknown>[] = [...rows];

  const existingTimestamps = new Set(rows.map((r) => r[timeKey] as string));

  // Fill defaults mirror the WITH FILL convention used upstream:
  // - Nullable columns: default = null → frontend shows "n/a" series
  // - Non-Nullable columns: default = "" → frontend filters it out
