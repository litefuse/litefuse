import {
  ...
  LITEFUSE_START_NODE_NAME,
  LITEFUSE_END_NODE_NAME,
} from "./types";
...
export function transformLanggraphToGeneralized(
  ...
  const transformedData = filteredData.map((obs) => {
    ...
    if (obs.node === LANGGRAPH_START_NODE_NAME) {
      transformedObs.name = LITEFUSE_START_NODE_NAME;
      transformedObs.id = LITEFUSE_START_NODE_NAME;
    } else if (obs.node === LANGGRAPH_END_NODE_NAME) {
      transformedObs.name = LITEFUSE_END_NODE_NAME;
      transformedObs.id = LITEFUSE_END_NODE_NAME;
    }
  ...
  const hasStartNode = transformedData.some(
    (obs) => obs.name === LITEFUSE_START_NODE_NAME,
  );
  const hasEndNode = transformedData.some(
    (obs) => obs.name === LITEFUSE_END_NODE_NAME,
  );
  ...
  if (!hasStartNode) {
    ...
    systemNodes.push({
      id: LITEFUSE_START_NODE_NAME,
      name: LITEFUSE_START_NODE_NAME,
      node: LITEFUSE_START_NODE_NAME,
      step: 0,
  ...
  if (!hasEndNode) {
    ...
    systemNodes.push({
      id: LITEFUSE_END_NODE_NAME,
      name: LITEFUSE_END_NODE_NAME,
      node: LITEFUSE_END_NODE_NAME,
      step: maxStep + 1,
      parentObservationId: topLevelObs?.parentObservationId || null,
      startTime: new Date().toISOString(),
      endTime: new Date().toISOString(),
...
export function buildGraphFromStepData(
  ...
  data.forEach((obs) => {
    ...
    if (obs.parentObservationId) {
      ...
      if (!parent && node === null) {
        if (!nodeToObservationsMap.has(LITEFUSE_END_NODE_NAME)) {
          nodeToObservationsMap.set(LITEFUSE_END_NODE_NAME, []);
        }
        nodeToObservationsMap.get(LITEFUSE_END_NODE_NAME)!.push(obs.id);
      }
  ...
  const nodeNames = [...new Set([...allStepNodes, LITEFUSE_END_NODE_NAME])];

  const nodes: GraphNodeData[] = nodeNames.map((nodeName) => {
    if (
      nodeName === LITEFUSE_END_NODE_NAME ||
      nodeName === LITEFUSE_START_NODE_NAME
    ) {
      return {
        id: nodeName,
        label: nodeName,
...
function generateEdgesWithParallelBranches(
  ...
  sortedSteps.forEach(([, currentNodes], i) => {
    ...
    Array.from(currentNodes).forEach((currentNode) => {
      ...
      if (
        currentNode === LITEFUSE_END_NODE_NAME ||
