...
export const projectsRouter = createTRPCRouter({
  ...
  delete: protectedProjectProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      const projectApiKeys = await ctx.prisma.apiKey.findMany({
        where: {
          projectId: input.projectId,
          scope: "PROJECT",
        },
      });
      await new ApiAuthService(ctx.prisma).deleteCachedApiKeys(
        projectApiKeys,
      ...
      await enqueuePgBossJob(QueueName.ProjectDelete, QueueJobs.ProjectDelete, {
        projectId: input.projectId,
        orgId: ctx.session.orgId,
      });
  ...
  transfer: protectedProjectProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      await new ApiAuthService(ctx.prisma).refreshCachedProjectApiKeys(
        input.projectId,
      );
    }),
