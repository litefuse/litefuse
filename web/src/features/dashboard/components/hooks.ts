...
function generateChartLabelFromColumns(
  ...
  return uniqueIdentifierColumns
    .map(({ accessor, formatFct }) => {
      ...
      // Skip empty strings as well as null/undefined: Doris fillTimeSeriesGaps
      // emits empty-string dimension fillers for non-nullable columns, and a
      // formatFct like `(${v.toLowerCase()})` would otherwise turn "" into "()".
      if (value === null || value === undefined || value === "") return null;
      return formatFct ? formatFct(value as string) : (value as string);
    })
