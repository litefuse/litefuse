...
function prepareScoresNumericV2Params(filter: FilterState) {
  ...
  // Fallback to 2000-01-01 instead of epoch 0 — DateTimeFilter passes
  // new Date(value).getTime() as the parameter, and the value 0 (epoch)
  // is rejected by the DateTime64(3) parameter parser.
...
/**
 * Converts the v2 histogram aggregate output to the { chartData, chartLabels }
 * shape returned by createHistogramData (used by the NumericScoreHistogram component).
 *
 * The aggregate returns an Array(Tuple(Float64, Float64, Float64))
...
async function getScoreAggregateV2({
  ...
  // filters manually: categorical scores always store value=0, so value=0
  // should include all categoricals, while any other value filter (e.g.
  // value=1) should exclude them. This matches v1 behavior where numeric
...
export const dashboardRouter = createTRPCRouter({
  ...
  scoreHistogram: protectedProjectProcedure
    ...
    .query(async ({ input }) => {
      if (input.version === "v2") {
        // v2: histogram() aggregates all matching rows server-side, so
