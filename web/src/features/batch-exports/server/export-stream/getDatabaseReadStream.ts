    ...
    case "observations": {
      ...
      return new DatabaseReadStream<unknown>(
        async (pageSize: number, offset: number) => {
          ...
          const fullObservations = await getObservationsById(
          ...
          const chunk: FullObservationsWithScores = generations.map(
            (generation) => {
              const fullObservation = fullObservations.find(
                (observation) => observation.id === generation.id,
              );
              ...
              return {
                ...generation,
                input: fullObservation?.input ?? generation.input,
                output: fullObservation?.output ?? generation.output,
                metadata: fullObservation?.metadata ?? generation.metadata,
                scores: outputScores,
              };
            },
          );
