 ...
 *
 * IMPORTANT: Because epoch-aligned interval bucketing produces undesirable
 * boundaries for multi-unit intervals, the backend ALWAYS queries with
 * SINGLE-UNIT intervals (1 second, 1 minute, 1 hour, 1 day, 1 month, 1 year).
 * This function receives single-unit data and aggregates it into the requested
 * multi-unit buckets, working backwards from toTimestamp to ensure "today's"
 * data appears in the rightmost bucket.
 *
 ...
 *
 * @param data - Array of SINGLE-UNIT time series data points from the backend
...
>(data: T[], fromDate: Date, toDate: Date, interval: IntervalConfig): T[] {
  ...
  // If single-unit interval (or 7-day weeks), the backend already returned the correct buckets
...
/**
 * Fill gaps in single-unit data (backend already returned correct bucket timestamps).
 */
...
/**
 * Aggregate single-unit data into multi-unit buckets.
