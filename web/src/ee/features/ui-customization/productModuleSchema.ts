 ...
 * - When LITEFUSE_UI_VISIBLE_PRODUCT_MODULES is set: only listed modules are visible
 * - When LITEFUSE_UI_HIDDEN_PRODUCT_MODULES is set: all modules except listed ones are visible
 *
 * If both are set, LITEFUSE_UI_VISIBLE_PRODUCT_MODULES takes precedence.
...
export function getVisibleProductModules(
  ...
  if (visibleModulesEnv && hiddenModulesEnv) {
    console.warn(
      "Both LITEFUSE_UI_VISIBLE_PRODUCT_MODULES and LITEFUSE_UI_HIDDEN_PRODUCT_MODULES are set. " +
        "Using LITEFUSE_UI_VISIBLE_PRODUCT_MODULES as the allow list.",
    );
