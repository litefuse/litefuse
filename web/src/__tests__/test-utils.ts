...
export const ensureTestDatabaseExists = async () => {
  ...
  // Doris uses the default database (no setup needed)
};

export const disconnectQueues = async () => {
  const { logger, stopPgBoss, stopSharedPostgresPool } = await import(
  ...
  try {
    ...
    logger.error(`Error stopping pg-boss in test teardown: ${error}`);
  }
  ...
  try {
    ...
    logger.error(
      `Error stopping shared postgres pool in test teardown: ${error}`,
    );
  }
};
...
export async function makeAPICall<T = IngestionAPIResponse>(
  ...
  const finalUrl = `http://localhost:${port}${url.startsWith("/") ? url : `/${url}`}`;
  const authorization =
