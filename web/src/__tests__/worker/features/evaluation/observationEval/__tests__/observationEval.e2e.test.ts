...
import { processObservationEval } from "@/src/server/background/features/evaluation/observationEval/observationEvalProcessor";
import {
...
describe("Observation Eval E2E Pipeline", () => {
  ...
  describe("full pipeline: schedule → process → execute", () => {
    it("should schedule and process observation through full eval pipeline", async () => {
      ...
      // ASSERT: Scheduling phase. Doris coordinates only — no S3 step.
      ...
      expect(pipeline.schedulerDeps.enqueueEvalJob).toHaveBeenCalledWith(
        expect.objectContaining({
          projectId,
      ...
      // ACT: Process the observation eval job (Doris re-read)
      ...
      expect(pipeline.processorDeps.fetchObservation).toHaveBeenCalledWith({
        projectId,
        spanId: observation.span_id,
        startTimeDate: TEST_START_TIME_DATE,
      });
