...
import { type ObservationEvalProcessorDeps } from "@/src/server/background/features/evaluation/observationEval/observationEvalProcessor";
import {
  type EvalExecutionDeps,
  createMockEvalExecutionDeps,
} from "@/src/server/background/features/evaluation/evalExecutionDeps";
 ...
 * The processor now reads from Doris instead of S3.
 */
export function createMockProcessorDeps(
  overrides: Partial<{
    fetchObservation: ReturnType<typeof vi.fn>;
  }> = {},
  ...
  return {
    fetchObservation:
      overrides.fetchObservation ??
      vi.fn().mockResolvedValue(defaultObservation),
  };
}
