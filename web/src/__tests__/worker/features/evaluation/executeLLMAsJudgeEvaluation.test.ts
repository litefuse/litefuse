...
import { createMockEvalExecutionDeps } from "@/src/server/background/features/evaluation/evalExecutionDeps";
import { UnrecoverableError } from "@/src/server/background/errors/UnrecoverableError";
import { ExtractedVariable } from "@/src/server/background/features/evaluation/observationEval/extractObservationVariables";
...
describe("executeLLMAsJudgeEvaluation", () => {
  ...
  describe("successful execution", () => {
    ...
    it("should use provided environment", async () => {
      const enqueueScoreIngestion = vi.fn();
      const deps = createSuccessfulDeps({ enqueueScoreIngestion });
  ...
  describe("score persistence errors", () => {
    it("should throw error if score upload fails", async () => {
      ...
      await expect(
        executeLLMAsJudgeEvaluation(createExecutionParams({ deps })),
      ).rejects.toThrow("Failed to direct-write score");
    });
  ...
  describe("prompt compilation", () => {
    it("should handle prompt compilation errors gracefully", async () => {
      ...
      expect(enqueueScoreIngestion).toHaveBeenCalled();
    });
  ...
  describe("execution metadata", () => {
    it("should include dataset item ID in metadata when present", async () => {
      ...
      const enqueueScoreIngestion = vi.fn();
      const deps = createSuccessfulDeps({ callLLM, enqueueScoreIngestion });
  ...
  describe("score event structure", () => {
    ...
    it("should use same scoreId in enqueueScoreIngestion and updateJobExecution", async () => {
      const enqueueScoreIngestion = vi.fn();
      const updateJobExecution = vi.fn();
      const deps = createSuccessfulDeps({
      ...
      const uploadCall = enqueueScoreIngestion.mock.calls[0][0];
