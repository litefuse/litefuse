...
function generateTraces(
  ...
  for (let i = 0; i < count; i++) {
    const timestamp = new Date(
      now.getTime() - Math.random() * numberOfDays * 24 * 60 * 60 * 1000,
    );
  ...
  for (let i = 0; i < count; i++) {
    ...
    observations.push({
      ...
      parent_observation_id:
        Math.random() > 0.7
          ? null
          : `obs_${Math.floor(Math.random() * i)}_${projectId}`,
      ...
      provided_usage_details:
        type === "GENERATION"
          ? `{"input": ${Math.floor(Math.random() * 1000)}, "output": ${Math.floor(Math.random() * 1000)}}`
          : "{}",
      usage_details:
        type === "GENERATION"
          ? `{"input": ${Math.floor(Math.random() * 1000)}, "output": ${Math.floor(Math.random() * 1000)}}`
      ...
      provided_cost_details:
        type === "GENERATION"
          ? `{"input": ${(Math.random() * 10).toFixed(6)}, "output": ${(Math.random() * 10).toFixed(6)}}`
          : "{}",
      cost_details:
        type === "GENERATION"
          ? `{"input": ${(Math.random() * 10).toFixed(6)}, "output": ${(Math.random() * 10).toFixed(6)}}`
          : "{}",
      total_cost:
        type === "GENERATION" ? (Math.random() * 20).toFixed(6) : null,
      completion_start_time: new Date(
        startTime.getTime() + Math.random() * 1000,
      )
        .toISOString()
        .replace("T", " ")
        .replace("Z", ""),
...
function generateScores(
  ...
  for (let i = 0; i < count; i++) {
    ...
    if (dataType === "NUMERIC") {
      ...
      stringValue = ["poor", "fair", "good", "very good", "excellent"][
        value - 1
      ];
    }
    ...
    scores.push({
      ...
      observation_id:
        Math.random() > 0.7
          ? `obs_${Math.floor(Math.random() * observationsCount)}_${projectId}`
          : null,
...
async function insertDataInBatches(
  ...
  for (let i = 0; i < data.length; i += batchSize) {
    ...
    try {
      ...
      console.log(
        `  Inserted batch ${Math.floor(i / batchSize) + 1}/${Math.ceil(data.length / batchSize)}`,
      );
    } catch (error) {
...
async function main() {
  ...
  try {
    ...
    console.log(
      `Generating data for ${projectIds.length} projects over ${opts.numberOfDays} days...`,
    );
    ...
    if (project1 && project2) {
      ...
      const sampleObservations = generateObservations(
        projectIds[0],
        50,
        10,
        opts.numberOfDays,
        ...
        completionStartTime: o.completion_start_time
          ? new Date(o.completion_start_time)
          : null,
