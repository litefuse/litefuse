...
export const isBatchExportApiDownloadOnlyStatus = (
  status: string | null | undefined,
): status is BatchExportStatus.READY_API | BatchExportStatus.API_ONLY =>
  status === BatchExportStatus.READY_API ||
  status === BatchExportStatus.API_ONLY;

export const isBatchExportDownloadReadyStatus = (
  status: string | null | undefined,
): status is
  | BatchExportStatus.READY
  | BatchExportStatus.READY_API
  | BatchExportStatus.API_ONLY =>
  status === BatchExportStatus.READY ||
  isBatchExportApiDownloadOnlyStatus(status);
