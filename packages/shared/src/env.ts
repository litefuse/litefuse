...
const EnvSchema = z.object({
  ...
  LITEFUSE_CACHE_API_KEY_TTL_SECONDS: z.coerce.number().default(300), // 5m
  ...
  DORIS_DB: z.string().default("litefuse"),
  ...
  LITEFUSE_INGESTION_DORIS_HTTP_MAX_SOCKETS: z.coerce
    ...
    .default(200),
  /**
   * Doris group_commit_interval_ms applied per Stream Load.
   *
   * Lower values shorten the delay until rows become query-visible at the
   * cost of more frequent Doris-internal commits.
   */
  LITEFUSE_DORIS_GROUP_COMMIT_INTERVAL_MS: z.coerce
    .number()
    .int()
    .positive()
    .default(1000),
  /**
   * Doris group_commit_data_bytes applied per Stream Load.
   *
   * Reaching either this size or the group commit interval triggers a commit.
   */
  LITEFUSE_DORIS_GROUP_COMMIT_DATA_BYTES: z.coerce
    .number()
    .int()
    .positive()
    .default(134217728),
  ...
  // Default ON since the Doris fork lightweight pass: pg-boss now owns
  // TraceUpsert eval scheduling (was sharded BullMQ) plus all scheduled
  // worker jobs. Turning it off means every trace ingestion logs an
  // ERROR (enqueue fails silently caught) and scheduled cron jobs stop.
  LITEFUSE_PG_BOSS_ENABLED: z.enum(["true", "false"]).default("true"),
  LITEFUSE_PG_BOSS_SCHEMA: z.string().default("pgboss"),
  LITEFUSE_PG_BOSS_MIGRATE: z.enum(["true", "false"]).default("true"),
  LITEFUSE_PG_BOSS_SCHEDULE_TZ: z.string().default("UTC"),
  LITEFUSE_PG_BOSS_CONNECTION_TIMEOUT_MS: z.coerce
    .number()
    .positive()
    .default(30_000),
  LITEFUSE_PG_BOSS_POOL_MAX: z.coerce.number().positive().default(5),
  ...
  // Enable cache-based tracking of projects using OTEL API to optimize queries.
