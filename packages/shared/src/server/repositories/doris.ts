...
/**
 * Query Doris with named parameters.
 */
...
/**
 * Execute a non-SELECT Doris command (DDL, DELETE, UPDATE).
 */
 ...
 *
 * Backed by mysql2's `connection.query(sql).stream()` so heap stays bounded
 * and the first row is delivered as soon as Doris starts emitting — critical
 * for large analytics exports (PostHog/Mixpanel historical syncs) that would
...
/**
 * Parse a Doris-returned date/datetime value (string or Date) into a Date.
 */
