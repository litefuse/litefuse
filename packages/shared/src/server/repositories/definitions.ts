...
export const eventRecordBaseSchema = z.object({
  ...
  // The previous Doris fork shape (a separate `metadata` Map plus
  // `metadata_hashes` + `metadata_long_values`) is replaced by the
  // flat two-array layout — same as `metadata_names` / `metadata_values`
  // in main's events_full.
