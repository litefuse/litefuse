...
async function getObservationByIdFromEventsTableInternal({
  ...
  const query = `
    ...
    -- reading the compatibility views, even though mysql protocol reads
    -- are correct. Query the base table directly; in OSS the views are
    -- pass-through aliases over events_full anyway.
    FROM events_full
    WHERE project_id = {projectId: String}
...
> => {
  ...
  const query = `
    SELECT
      ...
      e.metadata_names,
      e.metadata_values
    -- See getObservationByIdFromEventsTableInternal above: avoid the
    -- compatibility view on Doris HTTP reads.
    FROM events_full e
