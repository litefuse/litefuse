...
import { observationsTableUiColumnDefinitionsForDoris } from "../tableMappings";
...
export const getObservationsForTrace = async <IncludeIO extends boolean>(
  ...
  // Doris HTTP JSONEachRow returns NULL for most non-key columns when
  // reading the compatibility views. Query the base table directly;
  // in the OSS schema the views are pass-through aliases over
  // events_full anyway.
  const query = `
    SELECT
      span_id AS id,
...
const getObservationByIdInternal = async ({
  ...
  // Same Doris HTTP JSONEachRow caveat as getObservationsForTrace:
  // query the base table directly instead of the compatibility views.
  const query = `
    SELECT
      span_id AS id,
      trace_id,
...
export const getObservationsForBlobStorageExport = function (
  ...
  // Batch the stream to keep memory bounded while preserving the
  // existing streaming shape.
