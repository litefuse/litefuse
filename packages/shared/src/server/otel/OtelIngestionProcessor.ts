...
import { RequestWriteBuffer } from "../ingestion/RequestWriteBuffer";
...
export interface ObservationEvalCandidate {
...
export class OtelIngestionProcessor {
   ...
   */
  /**
   * Synchronous OTel direct-write path used by the web /api/public/otel
   * route after MinIO/BullMQ removal.
   *
   * Pipeline:
   *   1. Apply ingestion masking (EE) — fail-closed if invalid.
   *   2. Convert resourceSpans into IngestionEvent[] via
   *      processToIngestionEvents.
   *   3. Split into observations + traces.
   *   4. For each observation, IngestionService.mergeAndWrite into the
   *      caller-provided RequestWriteBuffer (writes events_full). For
   *      traces, hand off to processEventBatch which
   *      runs the same direct-write path under a fresh RequestWriteBuffer.
   *   5. If header-based or scope-based direct-event-write checks pass,
   *      also emit enriched event records via ingestionService.writeEventRecord.
   *   6. Caller awaits writer.flushAll() to issue one Stream Load per
   *      touched table (group_commit: sync_mode); SDK gets 200 only
   *      after Doris commits.
   *
   * Observation-level eval scheduling is coordinated by the web package
   * so shared keeps the monorepo dependency direction intact. This
   * method only collects candidate observations; the caller runs
   * scheduleDirectObservationEvals() after `writer.flushAll()` so
   * follow-up eval jobs only hit Doris once the row is committed.
   */
  async processSpansSync(params: {
    resourceSpans: ResourceSpan[];
    ingestionService: IngestionService;
    writer: RequestWriteBuffer;
    collectObservationEvalCandidate?: (
      candidate: ObservationEvalCandidate,
    ) => void;
  }): Promise<void> {
    const { resourceSpans, ingestionService, collectObservationEvalCandidate } =
      params;
    const projectId = this.projectId;

    // 1. Ingestion masking is an Enterprise Edition feature; OSS passes spans
    //    through unchanged.
    const parsedSpans = resourceSpans;

    // 2. Convert to IngestionEvent[].
    const events: IngestionEventType[] =
      await this.processToIngestionEvents(parsedSpans);
    ...
    const observations = events
      ...
      .flatMap((o) => {
        if (!o.success) {
          logger.warn(
            `Failed to parse otel observation for project ${projectId}: ${o.error}`,
            o.error,
          );
          return [];
        }
    ...
    recordIncrement("litefuse.ingestion.event", observations.length, {
      source: "otel",
    });
    recordDistribution("litefuse.ingestion.otel.trace_count", traces.length);
    recordDistribution(
      "litefuse.ingestion.otel.observation_count",
      observations.length,
    );

    // 4. Direct-write decision (headers preferred, scope as fallback).
    // Litefuse Lightweight: the /api/public/otel/v1/traces route already
    // hard-rejects requests that wouldn't pass checkHeaderBasedDirectWrite,
    // so in this fork headerBasedDirectWrite is always true and the
    // scope-version fallback below is effectively dead. We keep it to
    // stay structurally aligned with upstream langfuse-main.
    ...
    const writePath = useDirectEventWrite
      ? headerBasedDirectWrite
        ...
        : "direct_scope"
      : "dual";
    recordIncrement("litefuse.ingestion.otel.write_path", 1, {
      path: writePath,
    });

    // 5. Direct event-record writes — canonical observation writer for
    //    the OTel path. Enabled whenever useDirectEventWrite is true,
    //    which the route-level header gate guarantees for v4 Python /
    //    v5 JS / ingestion-version=4 traffic.
    //
    //    Litefuse Lightweight, post Phase B refactor: no synthetic
    //    `t-<trace_id>` trace rows. Each OTel span produces exactly one
    //    events_full row (the observation). Trace-level fields
    //    (trace_name, user_id, session_id, tags, release, public,
    //    bookmarked) are denormalized onto every observation row by
    //    `createEventRecord`; trace-level lookups use GROUP BY trace_id
    //    aggregation (see eventsTraceAggregation in repositories).
    //    Path 1 (`mergeAndWrite` for traces/observations) is intentionally
    //    skipped — it was the legacy Doris-fork design that wrote a
    //    synthetic trace row and clobbered observation fields via the
    //    Unique Key. Aligned with upstream langfuse-main, which never
    //    writes a separate trace row to events_full either.
    //
    //    The `traces` array (computed above for backward-compat with
    //    upstream's split) is dropped here — we do not call
    //    `processEventBatch(traces, ...)`.
    void traces;

    if (!useDirectEventWrite) return;

    const eventInputs = this.processToEvent(parsedSpans);
    if (eventInputs.length === 0) return;

    await Promise.all(
      eventInputs.map(async (eventInput) => {
        try {
          // fileKey is empty in the direct-write path (no S3 staging);
          // downstream consumers that referenced it have been retired.
          ...
          if (collectObservationEvalCandidate) {
            collectObservationEvalCandidate({
              eventRecord,
              startTimeDate: new Date(eventRecord.start_time)
                .toISOString()
                .split("T")[0],
            });
          }
          ...
          logger.error(
            `Failed to create or write event record for ${eventInput.spanId}`,
            error,
          );
        }
      }),
    ...
    // Upsert trace_sessions into Postgres for every distinct sessionId
    // touched by this batch. The session detail page (`sessions.byIdWithScores`
    // / `enforceSessionAccess` middleware) reads from PG to validate
    // existence and the `public` / `bookmarked` flags. Upstream
    // langfuse-main does the same upsert inside processTraceEventList
    // (worker/src/services/IngestionService/index.ts:670); Phase B
    // removed that path for OTel observations, so the direct-write
    // path has to keep the contract.
    //
    // Dedupe by sessionId so a batch with many spans on the same
    // session triggers one upsert. ON CONFLICT DO NOTHING preserves
    // any UI-set flags from a prior insert.
    const sessionsToUpsert = new Map<string, string>();
    for (const e of eventInputs as Array<{
      sessionId?: string;
      environment?: string;
    }>) {
      if (e.sessionId && !sessionsToUpsert.has(e.sessionId)) {
        sessionsToUpsert.set(e.sessionId, e.environment ?? "default");
      }
    }
    if (sessionsToUpsert.size > 0) {
      await ingestionService.upsertTraceSessions({
        projectId,
        sessions: Array.from(sessionsToUpsert, ([id, environment]) => ({
          id,
          environment,
        })),
      });
   ...
   * Initializes seen traces from app cache automatically on first call.
  ...
  async processToIngestionEvents(
    ...
    return await instrumentAsync(
      ...
      async (span) => {
        ...
        try {
          // Lazy initialization - load seen traces from shared cache if not already done
  ...
  private convertValueToPlainJavascript(value: Record<string, any>): any {
    ...
    // OTLP JSON encodes int64 as a JSON string (spec: int64 fields must be
    // serialized as decimal strings to survive JS number precision). The
    // protobuf path emits Long objects (handled above); the JSON path
    // lands here as a string. Upstream main misses this case — protobuf
    // is the common path so it rarely shows up in production, but every
    // curl-based OTLP JSON test produces empty usage maps without this.
    if (value.intValue && typeof value.intValue === "string") {
      const parsed = parseInt(value.intValue, 10);
  ...
  private extractEnvironment(
    ...
    for (const key of environmentAttributeKeys) {
      // Lower-case so the OTel direct-write path matches the ingestion
      // schema's `PublicEnvironmentName` (z.string().toLowerCase()...).
      // Previously the lowercase happened during schema validation in
      // path 1 (mergeAndWrite). Phase B routes OTel observations only
      // through path 2 (createEventRecord + writeEventRecord) which
      // bypasses that schema, so we apply the transform here directly.
      if (attributes[key]) {
        return String(attributes[key]).toLowerCase();
      }
      if (resourceAttributes[key]) {
        return String(resourceAttributes[key]).toLowerCase();
      }
    }
  ...
  /**
   * Get a set of trace IDs that have been seen recently (from app cache).
    ...
    try {
      const results = await Promise.all(
        [...traceIds].map(async (traceId) => {
          ...
          const inserted = await setAppCacheValueIfAbsent(key, "1", {
            ttlSeconds,
          });
      ...
      // Cache error will be captured by parent span, just log and continue
      logger.error("Cache operation failed in getSeenTracesSet:", error);
