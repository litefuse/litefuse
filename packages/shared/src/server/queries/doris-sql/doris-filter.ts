...
export class StringObjectFilter implements Filter {
  ...
  apply(): DbFilter {
    // events_full stores metadata as parallel ARRAY<String> columns
    // (metadata_names + metadata_values), not a MAP. When this filter targets
    // one of those fields, swap the MAP subscript for
    // element_at(values, array_position(names, key)). Mirrors the upstream
    // langfuse v4 parallel-array pattern (events_proto/events_core/events_full).
    const EVENTS_FULL_METADATA_FIELDS = new Set([
      "metadata",
      "experiment_metadata",
      "experiment_item_metadata",
    ]);

    // Accept either tablePrefix on the filter OR an inline "<prefix>.<field>"
    // value in this.field (legacy column mappings encode the prefix this way).
    const dotIdx = this.field.lastIndexOf(".");
    const inlinePrefix = dotIdx === -1 ? "" : this.field.slice(0, dotIdx);
    const bareField = dotIdx === -1 ? this.field : this.field.slice(dotIdx + 1);
    const effectivePrefix = this.tablePrefix
      ? this.tablePrefix + "."
      : inlinePrefix
        ? inlinePrefix + "."
        : "";
    ...
    const valueAccessor = EVENTS_FULL_METADATA_FIELDS.has(bareField)
      ? `element_at(${effectivePrefix}${bareField}_values, array_position(${effectivePrefix}${bareField}_names, '${escapedKey}'))`
      : `${effectivePrefix}${bareField}['${escapedKey}']`;

    let query: string;
    switch (this.operator) {
      case "=":
        query = `${valueAccessor} = '${escapedValue}'`;
        break;
      case "!=":
        query = `${valueAccessor} != '${escapedValue}'`;
        break;
      case "contains":
        query = `INSTR(${valueAccessor}, '${escapedValue}') > 0`;
        break;
      case "does not contain":
        query = `INSTR(${valueAccessor}, '${escapedValue}') = 0`;
        break;
      case "starts with":
        query = `STARTS_WITH(${valueAccessor}, '${escapedValue}')`;
        break;
      case "ends with":
        query = `ENDS_WITH(${valueAccessor}, '${escapedValue}')`;
        break;
      default:
        throw new Error(`Unsupported operator: ${this.operator}`);
    }
...
export class NumberObjectFilter implements Filter {
  ...
  apply(): DbFilter {
    ...
    // scores_avg is stored as Array<Struct(name, value)> in Doris.
    // array_filter iterates over the array and OR-matches: row matches if
    // ANY struct satisfies name=key AND value operator threshold.
    // struct_element(x, N) accesses struct fields positionally
    // (1=name, 2=value); the upstream CK equivalent is x.1 / x.2.
    return {
