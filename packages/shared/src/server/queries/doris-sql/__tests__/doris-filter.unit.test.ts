...
describe("StringObjectFilter", () => {
  // events_full's metadata is stored as parallel arrays (metadata_names +
  // metadata_values), so filters on `metadata` (and experiment_metadata /
  // experiment_item_metadata) emit element_at + array_position, not Map
  // subscript. Matches main's pattern for events_proto/events_core/events_full.
  it("should generate = query with parallel-array access", () => {
  ...
  it("should generate contains query with parallel-array access", () => {
    const filter = new StringObjectFilter({
      table: "traces",
      field: "metadata",
      operator: "contains",
  ...
  it("should apply table prefix to both array columns", () => {
    const filter = new StringObjectFilter({
      table: "traces",
      field: "metadata",
      operator: "=",
  ...
  it("should extract inline prefix from <prefix>.<field>", () => {
    // Legacy column mappings encode the prefix in `select`, e.g. "o.metadata".
    const filter = new StringObjectFilter({
      table: "observations",
      field: "o.metadata",
      operator: "=",
  ...
  it("should route experiment_metadata to its own parallel arrays", () => {
    const filter = new StringObjectFilter({
      table: "observations",
    ...
    expect(filter.apply().query).toBe(
      "element_at(o.experiment_metadata_values, array_position(o.experiment_metadata_names, 'k')) = 'v'",
    );
  });

  it("should fall back to Map subscript for non-events_full fields", () => {
    // Defense-in-depth for any future filter on a real Map column.
    const filter = new StringObjectFilter({
      table: "traces",
      field: "some_map",
      operator: "=",
    ...
    expect(filter.apply().query).toBe("some_map['k'] = 'v'");
  });
});
