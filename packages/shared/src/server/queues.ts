...
// Doris is the source of truth: the processor re-reads the row from
// events_full using (projectId, spanId, startTimeDate) at job run
// time. We used to pass an S3 path that pointed to a JSON dump taken
// at scheduling time — that round-trip is gone.
export const LLMAsJudgeExecutionEventSchema = z.object({
  projectId: z.string(),
  jobExecutionId: z.string(),
  spanId: z.string(),
  startTimeDate: z.string(), // YYYY-MM-DD partition key on events_full
});
...
export type TQueueJobTypes = {
  ...
  [QueueName.PostHogIntegrationQueue]: {
    ...
    name: QueueJobs.PostHogIntegrationJob;
  };
  ...
  [QueueName.MixpanelIntegrationQueue]: {
    ...
    id: string;
    payload: Record<string, never>;
    name: QueueJobs.MixpanelIntegrationJob;
  };
  ...
  [QueueName.CoreDataS3ExportQueue]: {
    ...
    id: string;
    payload: Record<string, never>;
    name: QueueJobs.CoreDataS3ExportJob;
  };
