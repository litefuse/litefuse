 ...
 * 23:45 and 00:15, and env.LITEFUSE_INGESTION_QUEUE_DELAY_MS otherwise.
...
const getDelay = (delay: number | null, source: "api" | "otel") => {
  ...
  if ((hours === 23 && minutes >= 45) || (hours === 0 && minutes <= 15)) {
    return env.LITEFUSE_INGESTION_QUEUE_DELAY_MS;
  }
  ...
  return Math.min(5000, env.LITEFUSE_INGESTION_QUEUE_DELAY_MS);
};
...
export const processEventBatch = async (
  ...
  await instrumentAsync({ name: "s3-upload-events" }, async () => {
    ...
    const results = await Promise.allSettled(
      Object.keys(sortedBatchByEventBodyId).map(async (id) => {
        ...
        const bucketPath = `${env.LITEFUSE_S3_EVENT_UPLOAD_PREFIX}${authCheck.scope.projectId}/${getDorisEntityType(type)}/${eventBodyId}/${key}.json`;
  ...
  const projectIdsToSkipS3List =
    env.LITEFUSE_SKIP_S3_LIST_FOR_OBSERVATIONS_PROJECT_IDS?.split(",") ?? [];
