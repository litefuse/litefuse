...
export class PromptService {
  ...
  constructor(
    ...
    if (cacheEnabled !== undefined) {
      ...
      this.cacheEnabled = env.LITEFUSE_CACHE_PROMPT_ENABLED === "true";
    }

    this.ttlSeconds = env.LITEFUSE_CACHE_PROMPT_TTL_SECONDS;
  }
  ...
  private async getCachedPrompt(
    ...
    try {
      ...
      const value = await getAppCacheValue<PromptResult>(key);
  ...
  private async cachePrompt(params: PromptParams & { prompt: PromptResult }) {
    try {
      ...
      await setAppCacheValue(key, params.prompt, {
  ...
  public async invalidateCache(
    ...
    await setAppCacheValue(this.getEpochKey(params), this.newEpochToken(), {
      ttlSeconds: this.epochTtlSeconds,
    });
  }
  ...
  private async getOrCreateEpoch(
    ...
    const currentEpoch = await getAppCacheValue<string>(epochKey);
    if (currentEpoch) return currentEpoch;

    const newEpoch = this.newEpochToken();
    const created = await setAppCacheValueIfAbsent(epochKey, newEpoch, {
      ttlSeconds: this.epochTtlSeconds,
    });
    ...
    return (await getAppCacheValue<string>(epochKey)) ?? newEpoch;
  }
