...
async function getTracesTableGeneric(props: FetchTracesTableProps) {
  ...
  // Litefuse writes real observation rows only. Root observations use
  // parent_span_id = '' and there is no separate synthetic trace row in
  // events_full, so observation aggregates can scan all rows as-is.
  const actualObservationPredicate = "1 = 1";
      ...
      observations_stats AS (
        ...
        FROM (
          ...
          FROM (
            ...
            AND ${actualObservationPredicate}
