...
describe("formatDataForDoris", () => {
  ...
  describe("events_full parallel-array values", () => {
    // Doris Stream Load JSON parser miscounts \" inside string array elements
    // and silently absorbs subsequent elements (verified against Doris 4.0.3
    // with the real OTel payload that ships with langfuse v4 GenAI spans).
    // formatDataForDoris must unstring JSON-shaped string values to native
    // objects so the outer JSON.stringify emits clean nested JSON instead.
    it("leaves plain string values unchanged", () => {
      const result = formatDataForDoris([
        {
          span_id: "x",
          metadata_names: ["k1", "k2"],
          metadata_values: ["plain", "another"],
        },
      ]);
      expect((result[0] as any).metadata_values).toEqual(["plain", "another"]);
    });

    it("parses JSON-object-shaped string values to native objects", () => {
      const arg = '{"command":"curl -s \\"wttr.in\\"","timeout":10}';
      const result = formatDataForDoris([
        {
          span_id: "x",
          metadata_names: ["k1", "k2"],
          metadata_values: ["plain", arg],
        },
      ]);
      const out = (result[0] as any).metadata_values;
    ...
    it("parses JSON-array-shaped string values to native arrays", () => {
      const arg = '[{"role":"user","text":"hi \\"there\\""}]';
      const result = formatDataForDoris([
        {
          span_id: "x",
          metadata_names: ["msgs"],
      ...
      expect(out[0][0]).toEqual({ role: "user", text: 'hi "there"' });
    });
      ...
      const result = formatDataForDoris([
        {
          span_id: "x",
          metadata_names: ["k1"],
          metadata_values: [malformed],
        },
      ]);
      expect((result[0] as any).metadata_values[0]).toBe(malformed);
    });

    it("applies the same fix to experiment_*_values arrays", () => {
      const result = formatDataForDoris([
        {
          span_id: "x",
          experiment_metadata_names: ["k"],
          experiment_metadata_values: ['{"a":1}'],
          experiment_item_metadata_names: ["k"],
          experiment_item_metadata_values: ["[1,2,3]"],
        },
      ]);
      expect((result[0] as any).experiment_metadata_values[0]).toEqual({
        a: 1,
      });
      expect((result[0] as any).experiment_item_metadata_values[0]).toEqual([
        1, 2, 3,
      ]);
    });

    it("does not touch the paired names array (names stay strings)", () => {
      const result = formatDataForDoris([
        {
          span_id: "x",
          metadata_names: ['{"looks":"like_json"}', "plain"],
          metadata_values: ["v1", "v2"],
        },
      ]);
      expect((result[0] as any).metadata_names).toEqual([
        '{"looks":"like_json"}',
        "plain",
      ]);
    });
