...
- `events_full` 中每个事件（包括根事件）计 1 个 observation unit；
- `scores` 中每条 score 计 1 个 score unit；
- 将组织下所有未删除 Project 的结果相加。

该口径与 Langfuse 的 `traces + observations + scores` 定义一致：根事件同时贡献 1 个 trace unit 和 1 个 observation unit。QA 固定测试数据的当前预期聚合结果为 **4 units**，首次上报和同一小时重放都必须保持 4，不得重复累计。
