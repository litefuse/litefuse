...
- Adding prices for Codex, OpenAI, Anthropic, Google, Gemini, AWS Bedrock, Azure, or Vertex AI models
...
|-------|------|---------|-------------|
| `tokenizerId` | string | null | "Codex", "openai", or null |
...
| Provider | URL |
|----------|-----|
| Anthropic Codex | https://platform.Codex.com/docs/en/about-Codex/pricing |
...
### Anthropic Codex (API + Bedrock + Vertex)

```regex
(?i)^(anthropic\/)?(Codex-opus-4-6|(eu\\.|us\\.|apac\\.)?anthropic\\.Codex-opus-4-6-v1(:0)?|Codex-opus-4-6)$
```

**Matches:**
- `Codex-opus-4-6` (direct API)
- `anthropic/Codex-opus-4-6` (with prefix)
- `anthropic.Codex-opus-4-6-v1:0` (AWS Bedrock)
- `us.anthropic.Codex-opus-4-6-v1:0` (regional Bedrock)
- `Codex-opus-4-6` (GCP Vertex)

### With Version Date

```regex
(?i)^(anthropic\/)?(Codex-opus-4-5-20251101|(eu\\.|us\\.|apac\\.)?anthropic\\.Codex-opus-4-5-20251101-v1:0|Codex-opus-4-5@20251101)$
```
...
| `@date` | Vertex AI format | `Codex-3-5-sonnet@20240620` |
...
```
WebFetch URL: https://platform.Codex.com/docs/en/about-Codex/pricing
...
```json
{
  ...
  "modelName": "Codex-opus-4-6",
  "matchPattern": "(?i)^(anthropic\/)?(Codex-opus-4-6|(eu\\.|us\\.|apac\\.)?anthropic\\.Codex-opus-4-6-v1(:0)?|Codex-opus-4-6)$",
...
**Placement**: Insert near related models (e.g., other Codex models together).
...
- `anthropicModels` - Anthropic Codex models
...
export const anthropicModels = [
  "Codex-sonnet-4-5-20250929",  // Keep existing first entry
  "Codex-haiku-4-5-20251001",
  "Codex-opus-4-6",              // Add new model here (not first!)
  "Codex-opus-4-5-20251101",
  // ... rest of models
] as const;
```
...
# Verify entry exists
jq '.[] | select(.modelName == "Codex-opus-4-6")' /path/to/default-model-prices.json
```
...
console.log(pattern.test("Codex-opus-4-6")); // true
console.log(pattern.test("anthropic/Codex-opus-4-6")); // true
console.log(pattern.test("anthropic.Codex-opus-4-6-v1:0")); // true
console.log(pattern.test("us.anthropic.Codex-opus-4-6-v1:0")); // true
```
...
|------------|---------------|-------|
| Anthropic Codex | `Codex-opus-4-5-20251101` | Full multi-provider pattern |
